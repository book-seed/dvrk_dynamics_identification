...
void get_C_function(const double* q, const double* dq, const double* ddq, const double* p, double* results) {
    ...
    //公共子表达式
    double x0 = 2*q1;
    double x1 = 1.0*p2*cos(x0) + 0.5*p5*sin(x0);
    
    results[0] = dq1*x1;
    results[1] = dq0*x1 + dq1*(-1.0*p3*sin(q1) + 1.0*p4*cos(q1));
    results[2] = -dq0*x1;
    results[3] = 0;
}