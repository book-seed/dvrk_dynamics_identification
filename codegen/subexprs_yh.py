...
class SubExprs(object):
    ...
    class _ordered_len(object):
        ...
        def insert(self, list_, item):
            l = len(item)
            over = l - (len(self.lenidxs) - 1)
            if over > 0:
                self.lenidxs.extend([self.lenidxs[-1]] * over)
            list_.insert(self.lenidxs[l], item)
            for j in range(l, len(self.lenidxs)):
                self.lenidxs[j] += 1

        def pop(self, list_, index):
            l = len(list_.pop(index))
            for j in range(l, len(self.lenidxs)):
                self.lenidxs[j] -= 1
    ...
    def collect(self, exprs):
        print(type(exprs))
        print(f"Is Basic instance: {isinstance(exprs, sympy.Basic)}")
    ...
    def get(self, exprs=None, symbols=None):
        ...
        def _find_repeated_subexprs(subexpr_):
            if subexpr_.is_Atom:
                symbs = [subexpr_]
            else:
                symbs = subexpr_.args
            ...
            if subexpr_.is_Atom:
                ...
                if symb in ivar_se:
                    if symb in tmpivs_ivs:
                        ...
                        subexpr_ = ivar_se[symb]
                        args = list(map(_get_subexprs, subexpr_.args))
                        subexpr_ = type(subexpr_)(*args)
                        if symb in repeated:
                            ivar_ = next(symbols)
                            ordered_iv_se[ivar_] = subexpr_
                            tmpivs_ivs[symb] = ivar_
                            return ivar_
                        else:
                            return subexpr_
                else:
    ...
    def collect(self, exprs):
        ...
        for expr in exprs:
            if expr.is_Atom or (-expr).is_Atom:
                ...
                print(f"tmp_dict: {self._subexp_iv}")
                out_exprs.append(iv)
