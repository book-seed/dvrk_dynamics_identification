...
def code_to_string(code, out_params, printer, indent='', realtype='',
    ...
    for c, out in enumerate(out_params):
        codestr += '\n'
...
def gen_py_func(code, out_params, func_params, func_name='func'):

    indent = 4 * ' '

    pycode = 'def ' + func_name + '('
    pycode += ', '.join(func_params)
    pycode += '):\n\n'

    for i, out in enumerate(out_params):
        pycode += indent + out + ' = [0]*' + str(len(code[1][i])) + '\n'

    pycode += '\n'

    mainpycode = code_to_string(code, out_params, lambdarepr, indent)

    pycode += mainpycode

    pycode += '\n' + indent + 'return '
    pycode += ', '.join(out_params)
...
def gen_c_func(code, out_params, func_params, func_name='func'):

    indent = 2 * ' '

    ccode = 'void ' + func_name + '( double* '

    ccode += ', double* '.join(out_params)

    ccode += ', const double* '
    ccode += ', const double* '.join(func_params)

    ccode += ' )\n{\n'

    mainccode = code_to_string(code, out_params, _ccode, indent, 'double', ';')
...
def gen_julia_func(code, out_params, func_params, func_name='func'):

    indent = 4 * ' '

    ccode = 'function ' + func_name + '('

    ccode += ', '.join(out_params)

    ccode += ', '
    ccode += ', '.join(func_params)
    ...
    mainccode = code_to_string(code, out_params, _juliacode, indent, '', '',
                               outidxoffset=1)
...
def code_to_func(lang, code, out_params, func_name, func_params, symb_replace):
    ...
    if symb_replace:
        ...
        for k, v in symb_replace.items():
            ...
            if isinstance(v, str):
                v = sympy.Symbol(v)
            simplified_replace[k] = v
        code = xreplace(code, simplified_replace)

    return gen_func(code, out_params, func_params, func_name)
